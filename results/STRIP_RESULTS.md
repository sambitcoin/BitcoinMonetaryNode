...
## One value that pins the whole result

Two nodes can both hold valid blocks and still disagree completely about what
was removed. The merkle check does not detect that — each node's blocks verify
against their own headers regardless. A chained commitment does.

    C_h = SHA256d( C_{h-1} ‖ block hash_h ‖ K_h )

where K is the record's body digest, covering everything stored for that block:
retained transactions, stored txids, and every filter entry describing a dropped
output. Chained from the first block in the range.

Computed over all 194,863 blocks of this run:

> ### C = `f0e4c825e753cc9469a2027425c9625801f2bf8662f2e6b3cdbc4647b8406b61`

**This is a reproducibility target.** Anyone stripping blocks 767,430–962,292
with these classification rules should arrive at exactly that string. If they do
not, we disagree somewhere, and the per-block chain in
[`results/commitments.csv`](../results/commitments.csv) locates the first block
where the two diverge. A 64-character comparison instead of diffing 247 GB.

Properties, verified in `monetary_commit.py`'s self-tests: deterministic,
order-sensitive, permanently divergent once two histories differ, resumable from
a checkpoint, and sensitive to truncation.

**C is derived from the store, not written during stripping.** A node therefore
cannot assert a commitment its own data does not support, and any reviewer can
recompute it independently. The run also re-verifies every body digest as it
goes, making it a third confirmation that the store is intact.

**What C is not.** It commits to stripping decisions, not to UTXO set state. It
answers "did we strip identically", not "do we hold identical UTXO sets" — the
full L/M accumulator described in the design needs a UTXO database and remains
unimplemented. C also confers no security of its own: it anchors to
proof-of-work only by reference, mixing in block hashes that real work committed
to. It is not proof of that work.
